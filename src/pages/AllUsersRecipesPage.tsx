...
import { AdminRecipe, RecipeSearchFilters } from '../types/types';
import { getAllUsersRecipes } from '../services/recipeService';
import { getFilteredRecipesForAdmin } from '../services/filterService';
...
import { navigateToAllUsersRecipesPage } from '../utils/navigateHelper';
...
export default function AllUsersRecipesPage() {
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    navigateToAllUsersRecipesPage(navigate, page);
  };
  ...
  const handleSearch = async (filters: RecipeSearchFilters) => {
    ...
    navigateToAllUsersRecipesPage(navigate, 1, filters);
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!hasAnyFilter) {
          const data = await getAllUsersRecipes();
          setRecipes(data);
          setUserPerformedSearch(false);
          setNoResultsFound(false);
        } else {
          const data = await getFilteredRecipesForAdmin(filters);
          setRecipes(data);
  ...
  return (
    <Container fluid className="py-1">
      <div className="d-flex flex-column flex-sm-row justify-content-between align-items-center align-items-sm-start mb-2">
        <h3 className=" display-6 small-display mb-0 ms-2 text-nowrap">Manage Recipes</h3>
        {(recipes.length > 0 || userPerformedSearch) && (
          <PageControls
      ...
      <CSSTransition
        ...
        nodeRef={filterRef}
      >
        <div ref={filterRef} className="mb-3">
          <RecipeFinder
